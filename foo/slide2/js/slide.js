(function($){
    ...
    //  页面单个轮播实例 单例写法
    /*var slide1 = {
        slidebox : $('.P_slide.one'),
        container : $('.P_slide_container.one'),
        count : $('.P_slide_img.one li').length,
        t : '',
        init : function(){
            this.foo();
            this.addEvent();
        ...
        foo : function(){
            var _this = this;
            _this.container.css('width',''+(_this.count * 245)+'');
            $('.P_slide_img.one').animate({left:-(_this.count-4)*245},(_this.count-4)*2500,function(){
                setTimeout(function(){
                    $('.P_slide_img.one').animate({left:0},(_this.count-4)*2500);
                },2000);
            });
        },
        addEvent : function(){
            ...
            t = setTimeout(function(){
                _this.foo();
                _this.addEvent();
            },(_this.count-4)*2500*2+2000);
            _this.slidebox.mouseenter(function(){
                $('.P_slide_img.one').pause();
            }).mouseleave(function(){
                $('.P_slide_img.one').resume();
            });
        }
    };
    slide1.init();*/
