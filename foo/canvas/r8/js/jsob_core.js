...
//5. core.js(jsob_core.js)中大写字母开头的属性和方法，不会被混淆
...
(function(){
	...
	$.E($, {
		...
		,VN: function(a){return isNaN(a) || a==='' || a===null}
		,R: Math.round

		//STRING
		//起始位置的一个或多个非字符(^\s+)
		,F: function(a,b){ return (''+a).indexOf(b)+1;}
		...
		,FUN: function(a){
			...
			*/
			...
			$.EACH(a.split($.SPLIT0), function(b){	//b = ['A=B@:C1@-C2@-C31@=C32', 'd=e@:f']
				...
				if($._funcs[x[0]]){
					$._funcs[x[0]](data, detail);	//(['C1', 'C2', 'C31@=C32'], [['C1'], ['C2'], ['C31', 'C32']])	|| (['f'], [['f']])
				}else if($._funcs.ALL){
					$._funcs.ALL(data, x[0]);		//(['C1', 'C2', 'C31@=C32'], 'A')			|| (['f'], 'd')
				}
			});
		}
		...
		,C: function(father, css, tag){	//father, tag 也行
			...
			var default_css = {P:'absolute', id: z.ID_ || '_'+($._guid++)};
		...
		,B: function(min, max, c){return Math.max(min, Math.min(max, c));}
		,P2: function(a){return a*a;}

		//10进制-> 16进制
		,HX: function(n){var z = $.R(n).toString(16);	return z.length==1 ? '0'+z : z}
		...
		,LOG: function(){
			...
			var z = t.getMinutes()+':'+t.getSeconds()+'.'+(''+(t.getMilliseconds()/1000).toFixed(3)).substring(2)+' ';
			z += ($.MS() - this.lms) +' : ';
			this.lms = $.MS();
			var y;
			for(var i=0, len=arguments.length; i<len; i++){
				y = arguments[i].toString();
	...
	$._init.prototype = {
		...
		,S: function(css, done){
			...
			for(var i in css){
				...
				//如何让简写和全称保持一致？
 				...
 				if(		i == 'T' ){z.top 			= x;}
				else if(i == 'L' ){z.left 			= x;}
				else if(i == 'W' ){z.width 			= x;}
				else if(i == 'H' ){z.height 		= x;}
				else if(i == 'R' ){z.right	 		= x;}
				else if(i == 'B' ){z.bottom 		= x;}
				else if(i == 'ML'){z.marginLeft 	= x;}
				else if(i == 'MT'){z.marginTop 		= x;}
				else if(i == 'MR'){z.marginRight	= x;}
				else if(i == 'MB'){z.marginBottom	= x;}
				else if(i == 'F' ){z.fontSize		= x;}
				else if(i == 'LH'){z.lineHeight		= x;}
				else if(i == 'A' ){
					if($.IS.I678){
						if($.IS.I8){
				...
				else if(i == 'C' ){z.color 			= y;}
				else if(i == 'D' ){z.display 		= y;}
				else if(i == 'O' ){z.overflow		= y;}
				else if(i == 'P' ){z.position		= y;}
				else if(i == 'Z' ){z.zIndex			= y;}
				else if(i == 'CS'){z.cursor			= y;}
				else if(i == 'TA'){z.textAlign		= y;}

				//font
				else if(i == 'FF'){z.fontFamily		= y;}
				else if(i == 'FW'){z.fontWeight 	= y;}

				//background
				else if(i == 'BG'){z.background		=  y;}
				else if(i == 'BP'){z.backgroundPosition	= y;}

				//border
				else if(i == 'BD'){z.border			= y;}
				else if(i =='BDB'){z.borderBottom	= y;}
				else if(i == 'BR'){z.borderRadius 	= x;}
				else if(i == 'BS'){z.boxShadow 		= z[$._pre+'BoxShadow'] = y;}
				...
				//css3D
					...
					z['transition']	= z[$._pre+'Transition']=
						'all '
						+(y/1000)+'s '
						+(t.EASE_ || 'ease-in-out')+' '
						+ (t.DL_ || 0)/1000 +'s';
				}
				else if(i == 'TF'){z['transformStyle'] 		= z[$._pre+'TransformStyle'] 	= y;}
				else if(i == 'PP'){z['perspective'] 		= z[$._pre+'Perspective'] 		= x;}
				else if(i == 'PO'){z['perspectiveOrigin']	= z[$._pre+'PerspectiveOrigin']	= y;}
				else if(i == 'PW' ||
						i == 'PH'
				){
					perspective = 1;
		...
		,H : function(){return this.S({D: 'none'});}
		,V : function(){return this.S({D: ''});}		//写成 d: '' 大多数情况下可行
		,VV: function(){return this.S({D: 'block'});}	//写成 d: '' 大多数情况下可行
		,isH: function(){return this.D_=='none'}
		,toggle: function(){return this.isH() ? this.V() : this.H();}
		,css: function(css){
			return this.S(css)
		}
		,val: function(a){
		...
		,previous: function(){return this.next(1)}
			...
			if(t[type_] != fn){	//如果反复绑定同一个监听事件，简化处理，否则，需要以下处理步骤
				fun = function(e){
					...
					if(e){
						...
						t.type = tp;
						t.target = e.target || e.srcElement;
						//t.event = e;
		...
		,DRAG: function(option){
			...
			if(!('x' in option)){ option.x = []; }
			if(!('y' in option)){ option.y = []; }
			var z = this;
			var w = z.goal = option.goal || z;

			if(option.x && w.FATHER.W_){
				w.dif_w = Math.min(0, w.FATHER.W_ - w.W_);
			}else{
				w.dif_w = 0;
			}

			if(option.y && w.FATHER.H_){
				w.dif_h = Math.min(0, w.FATHER.H_ - w.H_);
			}else{
				w.dif_h = 0;
			}
			...
			.mousemove(function(z){
				if(z.is_move){
					var css = {}
						...
						, y = option.y;
					if(x){
						w.speed_x = z.X - (z.L_X||0)
						css.L = w.L_ + w.speed_x;
					}else{
						w.speed_x = 0;
					}
					if(y){
						w.speed_y = z.Y - (z.L_Y||0)

						css.T = w.T_ + w.speed_y;
					}else{
						w.speed_y = 0;
					}
					w.S(css);
		...
		,hover: function(a, b){
			this
			.mouseout(function(z){
				return typeof(a)=='string' ? z.S({src:a}) : z.S(a);
			})
			.mouseover(function(z){
				return typeof(b)=='string' ? z.S({src:b}) : z.S(b);
			});
...
// $((function(d, w){	//禁止选中
	...
	// d.onstart 		= function(){return false};
	// d.onselect 		= function(){
		// w.getSelection
